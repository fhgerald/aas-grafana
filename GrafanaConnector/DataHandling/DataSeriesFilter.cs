...
namespace GrafanaConnector.DataHandling
{
	/// <summary>
	...
	internal static class DataSeriesFilter
	{
		...
		public static IEnumerable<DataPoint<double>> FilterDataPoints<T>(this IEnumerable<DataPoint<T?>> dataPoints, DateTime startDateTime, 
		...
		private static IEnumerable<DataPoint<double>> GetDataPointEnumerable((double Value, double Count)[] points, 
		...
		private static DataPoint<double> CreateDataPoint(ref (double Value, double Count) point, DateTime dateTime) =>
			new(dateTime, point.Count > 0 ? point.Value / point.Count : 0);
